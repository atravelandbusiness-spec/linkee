  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 p-6 md:p-8 pb-32">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-black text-slate-800 tracking-tight">Mi Contenido</h1>
        <button 
          onClick={async () => {
            setIsEnhancing(true);
            ...
            if (res) {
               setState(prev => ({
                ...prev,
                profile: { ...prev.profile, bio: res.enhancedBio },
      ...
      <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 space-y-8">
        <div className="flex flex-col md:flex-row gap-8 items-start">
          <div className="relative group shrink-0 mx-auto md:mx-0">
            <img 
              ...
              alt="Avatar" 
            />
            <label className="absolute inset-0 flex items-center justify-center bg-black/40 rounded-full opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity border-2 border-white/30">
          ...
          <div className="flex-1 w-full space-y-6">
            <div className="grid grid-cols-1 gap-6">
                <div className="bg-indigo-50/30 p-4 rounded-2xl border border-indigo-100/50">
                  <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest block mb-2">Nombre de Usuario (@)</label>
                  <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-white border border-indigo-100/50 rounded-xl p-3 pl-8 font-black text-slate-800 outline-none focus:ring-2 focus:ring-indigo-200 transition-all text-sm"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-100 rounded-xl p-3 font-bold text-slate-800 outline-none focus:ring-2 focus:ring-indigo-100 transition-all"
                  />
                </div>
            </div>
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Biografía / Bio</label>
              <textarea
                name="bio"
                value={state.profile.bio}
                onChange={handleProfileChange}
                placeholder="Cuenta algo sobre ti..."
                className="w-full bg-slate-50 border border-slate-100 rounded-xl p-4 text-slate-600 outline-none focus:ring-2 focus:ring-indigo-100 min-h-[100px] resize-none text-sm leading-relaxed"
              />
            </div>
          </div>
        </div>
      ...
      <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-5 ml-1">Mis Redes Sociales</label>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {state.profile.socials.map((social, idx) => (
            <div key={social.platform} className="flex items-center gap-3 bg-slate-50 rounded-xl p-3 border border-slate-100 hover:border-indigo-100 transition-all group">
